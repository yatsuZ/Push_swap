...
/*   Updated: 2023/04/28 23:52:43 by yatsu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_mayon	*find_this_next(t_pile *p, t_mayon *m)
{
	...
	while (current)
	{
		if ((next_m->index - m->index) > (current->index - m->index) && (current->index - m->index) >= 0)//je veux avoir l'index le plus proche mais supererieur de m
			next_m = current;
...
int	suivie_rangement(t_mayon *next_ma, t_mayon *mb)
{
	...
	ft_printf("next_ma->index = %d | mb->index= %d |  precendent_ma->index = %d\n", next_ma->index, mb->index, precendent_ma->index);
