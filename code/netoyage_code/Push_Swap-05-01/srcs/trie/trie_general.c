...
/*   Updated: 2023/06/01 04:17:29 by yatsu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/// trie 1ier sexieme
	...
	{
		if (!p->a->index || p->a->index >= (huitieme / 2))
			use_instruction(TRUE, p, 1, RA);
		else
			use_instruction(TRUE, p, 1, PB);
	}
	while (get_last_mayon(p->a)->index && get_last_mayon(p->a)->index < huitieme * 2)
...
///// trie 2 iem sezieme
	...
	{
		if (p->a->index >= (huitieme * 4) - (huitieme / 2))
			use_instruction(TRUE, p, 1, RA);
		else
			use_instruction(TRUE, p, 1, PB);
	}
	while (get_last_mayon(p->a)->index && get_last_mayon(p->a)->index <= huitieme * 4)
		use_instruction(TRUE, p, 2, RRA, PB);
}
